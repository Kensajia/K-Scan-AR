// const JSON_PATH = './assets/IndexSet2.json'; // COLOCAR PARA REGRESAR A ESTRUCTURA MODULAR NO CENTRAL
let sceneEl;
let controls;
...
let isRotating360 = false; // NUEVO: Flag para control de Mutex (giro 360)
// Variable global para el control del tiempo de gracia.
let gracePeriodTimer = null; 

// Función para generar la clase CSS única a partir del ID del ícono (ej. "SONIDO ON" -> "icon_sonido_on")
const createIconClass = (id) => {
    // Conversión: A minúsculas y reemplazar espacios por guiones bajos.
    // Esta función garantiza que tu CSS funcionará con la convención de nombres que proporcionaste.
    return `icon_${id.toLowerCase().replace(/ /g, '_')}`;
};


// === CONSTANTES DE ÍCONOS SVG ===

// Función auxiliar para transformar un Nombre de Propósito en una clase CSS (ej. "CARGA 3D" -> "icon_carga_3d")
const createCssClass = (name) => {
    // Convierte el nombre a minúsculas y reemplaza espacios por guiones bajos.
    // Esto coincide con la convención de su CSS: .icon_carga_3d
    return `icon_${name.toLowerCase().replace(/ /g, '_')}`;
};

/**
 * Genera el snippet SVG.
 * @param {string} id - El ID del elemento en el sprite SVG (ej. 'CARGA 1').
 * @param {string} [classPurpose=id] - El nombre que define la clase CSS (ej. 'CARGA 3D').
 */
const SVG_ICON = (id, classPurpose = id) => { 
    const customClass = createCssClass(classPurpose);
    return `<svg class="icon ${customClass}"><use xlink:href="#${id}"></use></svg>`;
};

// Gira a la Derecha (D) - Añade la clase 'spin-right'
const SVG_SPIN_D = (id, classPurpose = id) => { 
    const customClass = createCssClass(classPurpose);
    return `<svg class="icon spin-right ${customClass}"><use xlink:href="#${id}"></use></svg>`; 
};

// Gira a la Izquierda (I) - Añade la clase 'spin-left'
const SVG_SPIN_I = (id, classPurpose = id) => { 
    const customClass = createCssClass(classPurpose);
    return `<svg class="icon spin-left ${customClass}"><use xlink:href="#${id}"></use></svg>`;
};

// ➡️ ICONOS DE CARGA / ESTADO ESPECIAL
// const ICON_NOMBRE = SVG_ICON('NOMBRE SVG', 'PROPOSITO DEL SVG');
const ICON_INICIO = SVG_ICON('INICIO 1', 'INICIO');							// INICIO
const ICON_CARGA_EST = SVG_SPIN_D('CARGA 2', 'CARGA EST');					// CARGA ESTANDAR
const ICON_CARGA_3D = SVG_ICON('CARGA 1', 'CARGA 3D');						// CARGA 3D
const ICON_DESCARGA = SVG_ICON('DESCARGA', 'DESCARGA');						// DESCARGA
const ICON_SINCRONIZAR = SVG_SPIN_D('SINCRONIZAR', 'SINCRONIZAR');			// SINCRONIZAR

// ➡️ ICONOS DE ESTADO
const ICON_FLASH_NO = SVG_ICON('FLASH NO', 'FLASH NO');       			    // FLASH NO
const ICON_FLASH_OFF = SVG_ICON('FLASH OFF', 'FLASH OFF');       		    // FLASH OFF
const ICON_FLASH_ON = SVG_ICON('FLASH ON', 'FLASH ON'); 		        	// FLASH ON
const ICON_SONIDO_OFF = SVG_ICON('SONIDO OFF', 'SONIDO OFF');     		    // SONIDO OFF
const ICON_SONIDO_ON = SVG_ICON('SONIDO ON', 'SONIDO ON');           		// SONIDO ON
const ICON_VISIBLE_OFF = SVG_ICON('VISIBLE OFF', 'VISIBLE OFF'); 		   	// VISIBLE OFF
const ICON_VISIBLE_ON = SVG_ICON('VISIBLE ON', 'VISIBLE ON');         		// VISIBLE ON

// ➡️ ICONOS NEUTROS
const ICON_CONFIGURACION = SVG_SPIN_D('CONFIGURACION', 'CONFIGURACION');	// CONFIGURACION
const ICON_HD = SVG_ICON('HD', 'HD'); 					            		// HD
const ICON_SD = SVG_ICON('SD', 'SD');    						  			// SD
const ICON_REINICIAR = SVG_SPIN_I('REINICIAR', 'REINICIAR');       		    // REINICIAR
const ICON_SIGUIENTE = SVG_ICON('SIGUIENTE 1', 'SIGUIENTE');           		// SIGUIENTE
const ICON_INTERACCION = SVG_ICON('INTERACCION 1', 'INTERACCION');          // INTERACCION
const ICON_GIRANDO = SVG_SPIN_I('GIRANDO 1', 'GIRANDO');           			// GIRANDO
const ICON_GIRAR_FIN = SVG_ICON('GIRAR FIN', 'GIRAR FIN');         			// GIRAR FIN
const ICON_GIRAR = SVG_ICON('GIRAR', 'GIRAR');           					// GIRAR

// =======================================================

// === NUEVA FUNCIÓN: Carga el Sprite SVG Externo e Inyecta en el DOM ===
//async function loadSvgSprite(filePath = '../../assets/img/sprite.svg') { 
async function loadSvgSprite(filePath = window.USER_SVG_SPRITE_PATH || '../../assets/img/sprite.svg') { 
    try {
        // 1. Solicitar el contenido del archivo SVG
        const response = await fetch(filePath);
        
        if (!response.ok) {
            throw new Error(`Error al cargar el sprite SVG: ${response.statusText}`);
        }
        
        const svgText = await response.text();
        
        // 2. Inyectar el contenido al inicio del body (document.body)
        // Esto hace que los iconos estén disponibles para las referencias <use xlink:href="#ID">
        document.body.insertAdjacentHTML('afterbegin', svgText);
        
        console.log('SVG Sprite cargado y listo.');
    } catch (error) {
        console.error("No se pudo cargar el SVG Sprite:", error);
        // Opcional: Mostrar un mensaje de error visible al usuario si los iconos son críticos
        // alert("Error al cargar los íconos (SVG Sprite).");
    }
}
...
/**
 * Función auxiliar para obtener la entidad del modelo 3D activo.
 * @returns {HTMLElement|null} La entidad A-Frame del modelo.
 */
function getActiveModelEntity() {
    if (activeTargetIndex === null || !videoRotationState[activeTargetIndex]) return null;

    const { arEntities } = videoRotationState[activeTargetIndex];
    // Busca el modelo visible (el que tiene la clase '3d' o '3d+audio')
    return arEntities.find(entity => entity.getAttribute('visible')); 
}

/**
 * Detiene la rotación 360 y restaura la interfaz de usuario.
 */
function stop360Rotation() {
    const activeModel = getActiveModelEntity();
    if (!activeModel || !isRotating360) return;

    // 1. ELIMINAR LA ANIMACIÓN DE GIRO
    activeModel.removeAttribute('animation__rotate');

    // 2. SINCRONIZAR Y HABILITAR ROTACIÓN TÁCTIL (Touch-Rotation)
    // Sincroniza la posición actual del modelo con el componente touch-rotation para evitar saltos.
    const touchRotationComponent = activeModel.components['touch-rotation'];
    if (touchRotationComponent) {
        touchRotationComponent.syncCurrentRotationWithAttribute(); // Sincronizar posición final
        activeModel.setAttribute('touch-rotation', 'enabled', true); // Habilitar touch-rotation
    }
    
    // 3. RESTAURAR UI y BOTÓN
    btnRotate3D.innerHTML = `${ICON_GIRAR} ROTAR 3D`;
    btnRotate3D.style.background = 'var(--accent)'; // Volver a color de acento

    // Mostrar botones que estaban ocultos (si existen)
    s.htmlSelectors.btnFlash.classList.remove('hidden');
    // s.htmlSelectors.btnNextVideo.classList.remove('hidden'); // Descomentar si usas este botón

    // 4. RESTAURAR EL LISTENER y ESTADO
    btnRotate3D.removeEventListener("click", stop360Rotation);
    btnRotate3D.addEventListener("click", start360Rotation);
    isRotating360 = false;
    
    showStatusMessage(`${ICON_GIRAR_FIN} Giro detenido.`, 1000);
}


/**
 * Inicia la rotación continua (360 grados) del modelo 3D activo.
 */
function start360Rotation() {
    const activeModel = getActiveModelEntity();

    if (!activeModel || isRotating360) {
        // Ignorar si no hay modelo 3D activo o si ya está girando.
        if (isRotating360) {
            // Si ya está girando, el botón debe funcionar como un toggle para detener.
            stop360Rotation();
        }
        return;
    }

    // Obtener configuración de giro del dataset (establecido en initializeScene)
    const eje = activeModel.dataset.giraEje || 'Y'; // Por defecto 'Y' (vertical)
    const duracion = parseInt(activeModel.dataset.giraDuracion) || 1500; // Por defecto 1500ms

    // Obtener la rotación inicial para el punto de inicio
    const rotationString = activeModel.getAttribute('rotation') || '0 0 0';
    const [startX, startY, startZ] = rotationString.split(' ').map(Number);
    
    // 1. CONFIGURACIÓN DE ESTADO Y UI
    isRotating360 = true;
    showStatusMessage(`${ICON_GIRANDO} Iniciando giro 360...`, 0); // Mensaje fijo

    // Deshabilitar botones que no se deben usar durante la rotación
    s.htmlSelectors.btnFlash.classList.add('hidden');
    // s.htmlSelectors.btnNextVideo.classList.add('hidden'); // Descomentar si usas este botón
    
    // Cambiar el ícono y estilo del botón para indicar estado activo
    btnRotate3D.innerHTML = `${ICON_GIRAR_FIN} DETENER GIRO`;
    btnRotate3D.style.background = 'var(--danger)'; // Usar color de peligro para indicar cambio

    // 2. DESHABILITAR ROTACIÓN TÁCTIL (Mutex)
    activeModel.setAttribute('touch-rotation', 'enabled', false);

    // 3. APLICAR ANIMACIÓN DE A-FRAME
    
    let toValue = '';
    // Determinar el valor final de la animación (rotación completa)
    switch (eje.toUpperCase()) {
        case 'X':
            toValue = `${startX + 360} ${startY} ${startZ}`;
            break;
        case 'Z':
            toValue = `${startX} ${startY} ${startZ + 360}`;
            break;
        case 'Y':
        default:
            toValue = `${startX} ${startY + 360} ${startZ}`;
            break;
    }

    activeModel.setAttribute('animation__rotate', {
        property: 'rotation',
        to: toValue,
        dur: duracion,
        easing: 'linear',
        loop: true // Rotación infinita hasta que se detenga
    });

    // 4. RESTAURAR EL LISTENER
    btnRotate3D.removeEventListener("click", start360Rotation);
    btnRotate3D.addEventListener("click", stop360Rotation);
}
...
async function loadConfig() {
	// 1. Usar la variable global definida en el HTML
    const JSON_PATH = window.USER_JSON_PATH;
    try {
		// 2. Ejecutar el fetch con la ruta dinámica
        const response = await fetch(JSON_PATH);
        if (!response.ok) {
            throw new Error(`Network error or IndexSet2.json not found`);
			// throw new Error(`Error HTTP: ${response.status}`); // COLOCAR PARA REGRESAR A ESTRUCTURA MODULAR NO CENTRAL Y ELIMINAR LA OTRA
        }
        config = await response.json();
        console.log("Configuración AR cargada correctamente.");
    ...
    Targets.forEach(target => {
        ...
        elementos.forEach((contentData, index) => {
            ...
            if (is3DModel) {
                ...
                // modelAsset.setAttribute('src', contentData.src); // URL no codificada
				modelAsset.setAttribute('src', decodeURL(contentData.src)); // URL codificada
                ...
                //videoEntity.dataset.videoSrc = contentData.src; // URL no codificada
				videoEntity.dataset.videoSrc = decodeURL(contentData.src); // URL codificada
...
function finalize360Rotation() {
    ...
    if (currentEntity && currentEntity.tagName === 'A-ENTITY' && currentEntity.hasAttribute('gltf-model')) {
		...
		setTimeout(function() {
			showStatusMessage(`${ICON_INTERACCION} Puedes interactuar.`, 500);
		}, 1000);
    }
}
// ===============================================================
...
function start360Rotation() {
    ...
    showStatusMessage(`${ICON_GIRANDO} Girando...`, 0); // Fijo hasta que termine la animación
...
function playCurrentContent(targetIndex) {
    ...
    if (currentEntity.tagName === 'A-VIDEO' || currentEntity.tagName === 'A-PLANE') {
        ...
        if (!currentVidAsset.dataset.loadedSrc || currentVidAsset.dataset.loadedSrc !== currentUrl) {
            ...
            loadTimer = setTimeout(() => {
                showStatusMessage(`${ICON_CARGA_EST} Cargando contenido…`);
            }, 1000); 
        }
    ...
    // --- Lógica para Modelos 3D (Estático o Animado SIN audio asociado) ---
        ...
        const loadingHandle = setTimeout(() => {
            showStatusMessage(`${ICON_CARGA_3D} Cargando modelo 3D...`);
        }, 1000); 
    ...
    // --- Lógica para Modelos 3D con Audio Asociado (TIPO 3D+AUDIO) ---
        ...
        // const currentUrl = contentData.audioSrc; // URL no codificada
		const currentUrl = decodeURL(contentData.audioSrc); // URL codificada
...
function initializeUIListeners() {
	...
	showStatusMessage(`${ICON_INICIO} Iniciando experiencia AR…`, 4000);
    ...
    btnFlash.addEventListener("click", function() {
        if (trackRef.track && !this.disabled) {
            ...
            trackRef.track.applyConstraints({ advanced: [{ torch: !isCurrentlyOn }] }).then(() => {
                ...
                this.innerHTML = !isCurrentlyOn ? `${ICON_FLASH_ON} FLASH ON` : `${ICON_FLASH_OFF} FLASH OFF`;
            }).catch(error => {
...
// Función para limpiar y decodificar la URL del JSON
function decodeURL(encoded) {
    if (!encoded) return "";

    let text = encoded.trim();

    // 1. Detectar si ya es una URL normal (No codificada)
    if (text.toLowerCase().startsWith("http") || text.toLowerCase().startsWith("./assets")) {
        console.warn("⚠️ [Seguridad] La URL se cargó en texto plano (sin codificar):");
        return text;
    }

    try {
        // 2. Intentar decodificar Base64
        // btoa/atob solo funcionan bien con caracteres ASCII (típicos de URLs)
        const decoded = atob(text);
        
        // Verificación extra: ¿El resultado de la decodificación parece una URL?
        if (decoded.includes("http") || decoded.includes("/")) {
            return decoded;
        } else {
            console.warn("⚠️ El texto decodificado no parece una URL válida.");
            return text;
        }
    } catch (e) {
        // 3. Si atob falla, significa que el texto no es Base64 válido
        // En lugar de romper el código, lo devuelve tal cual para que el AR intente cargarlo
        console.error("❌ Error: El valor en el JSON no es Base64 válido ni una URL clara:");
        return text;
    }
}
...
async function initApp() {
    // 1. ES CRUCIAL: Esperar a que el sprite se cargue en el DOM
    await loadSvgSprite();
    
    // 2. Ahora es seguro llamar a initializeSelectors porque los íconos están presentes
    initializeSelectors(); 
    
    // 3. Continuar con la inicialización existente (que ya no necesita el evento DOMContentLoaded)
    initializeUIListeners();
    loadConfig(); 
}


// --- INICIO DEL CÓDIGO (NUEVO BLOQUE DE EJECUCIÓN) ---

// Ejecutar la aplicación principal SOLO después de que el DOM esté completamente cargado.
// Esto garantiza que el <body> y los elementos de la UI existan antes de llamar a initApp().
